...
class HomeActivity : AppCompatActivity() {
    ...
    private var listMakanan = listOf<DetailsMakanan>()
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        listMakanan = getdata()
        binding.buttonCari.setOnClickListener { cariMakanan() }

    }

    private fun getdata(): List<DetailsMakanan> {
        return listOf(
            DetailsMakanan(
                "Pasta",
    ...
    private fun cariMakanan() {
        val search = binding.pencarianInp.text.toString().toLowerCase()
        if (TextUtils.isEmpty(search)) {
            Toast.makeText(this, R.string.cari_invalid, Toast.LENGTH_LONG).show()
        }

        //   untuk memisahkan tiap data menjadi index yang berbeda, agar bisa dipanggil
        val food = listMakanan.find { it.nama.equals(search, ignoreCase = true) }

        if (food != null) {
            binding.gambar.setImageResource(food.imageResId)
            binding.gambar.visibility = View.VISIBLE
            binding.nama.text = food.nama
            binding.nama.visibility = View.VISIBLE
            binding.deskripsiResep.text = food.deskripsi
            binding.deskripsiResep.visibility = View.VISIBLE
        } else {
            binding.deskripsiResep.visibility = View.VISIBLE
            binding.nama.text = getString(R.string.data_invalid)
            binding.gambar.visibility = View.GONE
            binding.nama.visibility = View.GONE
            binding.deskripsiResep.visibility = View.GONE
        }
